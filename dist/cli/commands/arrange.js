...
import { alignElements, distributeElements, setElementsLocked, groupElements, ungroupElements, duplicateElements } from '../../core/geometry.js';
const ALIGNMENTS = new Set(['left', 'center', 'right', 'top', 'middle', 'bottom']);
const DIRECTIONS = new Set(['horizontal', 'vertical']);
function parseIds(value, usage) {
    if (typeof value !== 'string' || !value.trim()) {
        throw new CliUsageError(usage);
    }
    return value.split(',').map(s => s.trim()).filter(Boolean);
}
export async function arrange(argv) {
    ...
    switch (op) {
        case 'align': {
            ...
            printJson(await alignElements(ids, to));
            return;
        }
        case 'distribute': {
            ...
            printJson(await distributeElements(ids, to));
            return;
        }
        case 'group': {
        ...
        case 'ungroup': {
            ...
            if (!groupId)
                throw new CliUsageError('Usage: arrange ungroup --group <groupId>');
