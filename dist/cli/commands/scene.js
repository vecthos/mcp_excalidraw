...
import { getElements, clearCanvas, exportImage, sendMermaid } from '../../core/canvas-client.js';
...
async function readTextFileOrStdin(inputPath) {
...
export async function screenshot(argv) {
...
export async function exportCmd(argv) {
    const { flags } = parseArgs(argv, {
        out: { takesValue: true },
        format: { takesValue: true }
    });
...
export async function mermaid(argv) {
...
export async function share(argv) {
    parseArgs(argv, {});
    await ensureCanvasRunning();
    const elements = await getElements();
...
export async function clear(argv) {
