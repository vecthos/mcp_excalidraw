...
function resolveSkillsRoot(target) {
    if (target === 'claude')
...
export async function installSkill(argv) {
    ...
    const targetSpec = flags.target ?? 'claude';
    ...
    if (lstat?.isSymbolicLink()) {
        throw new Error(`${target} is a symlink; refusing to replace it. Remove it manually if you want the CLI to manage this install.`);
    }
