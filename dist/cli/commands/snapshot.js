...
import { saveSnapshot, listSnapshots, getSnapshot, clearCanvas, batchCreateElementsStrict } from '../../core/canvas-client.js';
export async function snapshot(argv) {
    ...
    switch (action) {
        case 'save': {
            if (!name)
                throw new CliUsageError('Usage: snapshot save <name>');
        ...
        case 'restore': {
            if (!name)
                throw new CliUsageError('Usage: snapshot restore <name>');
            let snap;
            try {
