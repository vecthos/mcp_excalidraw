...
export async function requireBrowserClient(what) {
    const health = await getHealth();
    if (health.websocket_clients === 0) {
        const error = new Error(`${what} requires the canvas to be open in a browser. Open ${EXPRESS_SERVER_URL} and retry.`);
        error.code = 'BROWSER_REQUIRED';
        throw error;
    }
}
// Read JSON input from a positional file argument or stdin ("-" = stdin).
export async function readJsonInput(file, what) {
    ...
    try {
        ...
        throw new CliUsageError(`Invalid JSON ${what}: ${error.message}`);
    }
}
