...
function unreachableError(reason) {
    const error = new Error(`Canvas server is not reachable at ${EXPRESS_SERVER_URL} (${reason}). ` +
...
export function isCanvasHealth(health) {
 ...
 * answers.
 */
export async function ensureCanvasRunning(options = {}) {
    ...
    while (Date.now() < deadline) {
        if (isCanvasHealth(await healthOrNull(400))) {
            markCanvasIdentityVerified();
            process.stderr.write(`Canvas server running at ${EXPRESS_SERVER_URL} — open it in a browser for screenshots and mermaid conversion.\n`);
 ...
 */
export async function stopCanvas() {
    ...
    const pid = isCanvasHealth(health) && Number.isSafeInteger(health.pid) && health.pid > 0
        ? health.pid
        : null;
    if (pid === null) {
        throw foreignServiceError();
    ...
    try {
        ...
        throw new Error(`Failed to signal canvas server (pid ${pid}): ${error.message}`);
    }
