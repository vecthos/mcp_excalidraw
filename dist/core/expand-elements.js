...
export function canonicalizeKeys(v) {
    if (Array.isArray(v))
...
export function expandElementsForExport(sourceElements, options = {}) {
    const { deterministic = false } = options;
    const seedFor = (key) => deterministic ? (fnv1a(key) % 2147483646) + 1 : Math.floor(Math.random() * 2147483647);
    const updatedFor = (el) => {
        if (!deterministic)
            return Date.now();
    ...
    for (const el of sourceElements) {
        ...
        const hasBoundText = Array.isArray(base.boundElements) &&
            base.boundElements.some((b) => b?.type === 'text');
    ...
    for (const el of cleanedExportElements) {
        ...
        if (arrowBindings) {
            ...
            const existing = new Set((Array.isArray(el.boundElements) ? el.boundElements : []).map((b) => b?.id));
...
//# sourceMappingURL=expand-elements.js.map