...
import { updateElementOnCanvas, deleteElementOnCanvas, getElementFromCanvas, createElementOnCanvas, batchCreateElementsOnCanvas, getElements, searchElements, clearCanvas, exportImage, setViewport, saveSnapshot, getSnapshot, sendMermaid } from './canvas-client.js';
import { sanitizeFilePath, prepareElement, prepareElementUpdate } from './normalize.js';
import { alignElements, distributeElements, setElementsLocked, groupElements, ungroupElements, duplicateElements } from './geometry.js';
 ...
 */
export async function callExcalidrawTool(name, args) {
    try {
        ...
        switch (name) {
                ...
                try {
                    ...
                    if (bbox) {
                        if (bbox.x_min !== undefined)
                            queryParams.set('x_min', String(bbox.x_min));
                        if (bbox.x_max !== undefined)
                            queryParams.set('x_max', String(bbox.x_max));
                        if (bbox.y_min !== undefined)
                            queryParams.set('y_min', String(bbox.y_min));
                        if (bbox.y_max !== undefined)
                            queryParams.set('y_max', String(bbox.y_max));
                    }
            ...
            case 'batch_create_elements': {
                ...
                const createdElements = params.elements.map(elementData => prepareElement(elementData));
            ...
            case 'duplicate_elements': {
                ...
                const { duplicates, canvasElements, offsetX, offsetY } = await duplicateElements(params.elementIds, params.offsetX ?? 20, params.offsetY ?? 20);
                return {
                    content: [{
                            type: 'text',
...
//# sourceMappingURL=mcp-dispatch.js.map